...
(async () => {
  ...
  // Fetch docs (all or new)
  const newDocs = await fetchDocs()
  console.log(`🔍 Fetched ${newDocs.length} doctor records.`)

  // Load existing if meta exists
  let allDocs = []
  if (fs.existsSync(doctorsDir) && fs.existsSync(metaFile)) {
    // read old pages
  ...
  console.log(`✅ Sitemaps generated: static, ${pages} doctor pages, index.`)
  process.exit(0)
})().catch(err => {
  console.error(err)
  process.exit(1)
